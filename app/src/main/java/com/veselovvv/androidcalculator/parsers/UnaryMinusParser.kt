...
interface UnaryMinusParser<T> {
    ...
    abstract class Abstract<T>(private val itemArrayRemover: ItemArrayRemover<T>) : UnaryMinusParser<T> {
        protected fun parse(
            operands: Array<String>, indexOperands: Array<Int>, tempArray: () -> Array<T>
        ): Array<T> {
            var resultArray = tempArray.invoke()

            if (operands[0] == SUBTRACT_OPERAND && indexOperands[0] == 0)
            ...
            indexOperands.forEachIndexed { index, element ->
                if (index <= indexOperands.lastIndex - 1) {
                    if (indexOperands[index + 1] - element == 1 && (operands[index + 1] == SUBTRACT_OPERAND))
